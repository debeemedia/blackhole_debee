...
async function login (req, res) {
    try {
        ...
        if(!password && (!email || !username)) {
            return res.status(400).json({success: false, message: 'Please provide your login details'})
        }
        ...
        bcrypt.compare(password, user.password, (err, result) => {
            if (result === true) {
                ...
                // issue refresh token
                const refreshToken = jwt.sign({id: user._id, email: user.email, username: user.username}, process.env.REFRESH_TOKEN_SECRET, {expiresIn: '1h'})
